...
class CallStats(object):
    ...
    def display(self, OUT=None, percent_threshold=None):
        if percent_threshold is not None:
            if self.pcnt_time_in_proc <= percent_threshold:
                return
                
        print>>OUT, '%6.2f %9.2f %9.2f  %s' % (self.pcnt_time_in_proc,
                                         self.cum_secs_in_proc,
                                         self.self_secs_in_proc,
                                         self.name)

def clean_coverage_count(OUT=None):
    global call_data
    l = call_data.items()
    d = {}
    for k, v in l:
        if os.path.basename(v.filename) == 'coverage.py':
            print>>OUT, '** REMOVING coverage.py'
            state.sample_count -= v.self_sample_count
            continue
        d[k] = v

    call_data = d

def display(OUT=None, percent_threshold=None):
    if state.sample_count == 0:
        print>>OUT, 'No samples recorded.'
        return
    ...
    print>>OUT, '%5.5s %10.10s   %7.7s  %-8.8s' % ('%  ', 'cumulative', 'self', '')
    print>>OUT, '%5.5s  %9.9s  %8.8s  %-8.8s' % ("time", "seconds", "seconds", "name")

    for x in l:
        x.display(OUT, percent_threshold=percent_threshold)

    print>>OUT, '---'
    print>>OUT, 'Sample count: %d' % state.sample_count
    print>>OUT, 'Total time: %f seconds' % state.accumulated_time
